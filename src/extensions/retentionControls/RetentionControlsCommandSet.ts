...
export default class RetentionControlsCommandSet extends BaseListViewCommandSet<IRetentionControlsCommandSetProperties> {
  ...
  private openRetentionControls = async (listItemIds: number[]): Promise<void> => {
    if (!this.context.pageContext.list?.id) {
      return;
    }

    const dialog = new RetentionControlsDialog(this.context, this.context.pageContext.list.id.toString(), listItemIds);
    await dialog.show();
  }
  ...
  public onExecute(event: IListViewCommandSetExecuteEventParameters): void {
    ...
    event.selectedRows.forEach(row => {
      listItemIds.push(parseFloat(row.getValueByName("ID")));
    });

    this.openRetentionControls(listItemIds).catch(console.error);
  }
